...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    protected function checkUploadedFile(array $aFile=null) {
	   ...
	   $upload_maxsize = "1MB";
       $uploaded_file_size=((filesize($file['tmp_name']) * .0009765625) * .0009765625);
	   
       if ( $uploaded_file_size > $GLOBALS['upload_maxsize'] )
            return $this->label("The file you are uploading is too big. Please use a smaller file of maximum size ".$GLOBALS['upload_maxsize']."Mb and try again.");	   
	   else if ($file['error'])
            return
